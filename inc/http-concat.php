...
/* Config */
global $concat_types,$concat_max_files,$concat_unique;
...
function concat_get_path( $uri ) {
	...
	$path                = ABSPATH;
	$stylesheet_url_path = concat_url_part( get_stylesheet_directory_uri() );
	$template_url_path   = concat_url_part( get_template_directory_uri() );
	$plugin_url_path     = concat_url_part( plugins_url() );


	if ( concat_starts_with( $uri, $stylesheet_url_path ) ) {

		$uri  = str_replace( $stylesheet_url_path, "", $uri );
		$path = STYLESHEETPATH;
	} else if ( concat_starts_with( $uri, $template_url_path ) ) {
		$uri  = str_replace( $template_url_path, "", $uri );
		$path = TEMPLATEPATH;
	} else if ( concat_starts_with( $uri, $plugin_url_path ) ) {
		$uri  = str_replace( $plugin_url_path, "", $uri );
		$path = WP_PLUGIN_DIR;
	}

	return $path . ( '/' != $uri[0] ? '/' : '' ) . $uri;
}

function concat_url_part($url){
	return parse_url($url, PHP_URL_PATH);
}

function concat_starts_with($haystack, $needle) {
